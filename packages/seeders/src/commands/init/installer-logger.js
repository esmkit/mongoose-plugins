...
export default class InstallerLogger extends BaseLogger {
	next({ type, payload }) {
		switch (type) {
			...
			case Installer.operations.CREATE_CUSTOM_SEEDER_TEMPLATE_FILE_SKIP_FILE_EXISTS:
				console.log(`${chalk.yellow("SKIP")} ${payload.customSeederTemplateFilename} are already exists`);
				break;
			case Installer.operations.CREATE_CUSTOM_SEEDER_TEMPLATE_FILE_SUCCESS:
				console.log(`${chalk.green("CREATED")} ${payload.customSeederTemplateFilename}`);
				break;

			case Installer.operations.CREATE_SEEDERS_FOLDER_SKIP_FOLDER_EXISTS:
				console.log(`${chalk.yellow("SKIP")} ${payload.foldername} are already exists`);
				break;
			...
			case Installer.operations.WRITE_USER_CONFIG_SKIP_FILE_EXISTS:
				console.log(`${chalk.yellow("SKIP")} ${payload.filename} are already exists`);
				break;
	...
	error({ type, payload }) {
		switch (type) {
			case Installer.operations.WRITE_USER_GENERETOR_CONFIG_ERROR:
				console.log(`${chalk.red("ERROR")} Unable to write config file: ${chalk.gray(payload.filepath)}`);
				break;

			case Installer.operations.CREATE_CUSTOM_SEEDER_TEMPLATE_FILE_ERROR:
				console.log(`${chalk.red("ERROR")} Unable to create custom seeder template: ${chalk.gray(payload.customSeederTemplatePath)}`);
				break;

			case Installer.operations.CREATE_SEEDERS_FOLDER_ERROR:
				console.log(`${chalk.red("ERROR")} Unable to create seeders folder: ${chalk.gray(payload.folderpath)}`);
				break;

			case Installer.operations.WRITE_USER_CONFIG_ERROR:
				console.log(`${chalk.red("ERROR")} Unable to write user config file: ${chalk.gray(payload.filepath)}`);
				break;
		}
