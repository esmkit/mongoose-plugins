...
function getPathnames(options, schema) {
	const paths = pathsFromTree(schema.tree);
...
function testSchema(schema, key, doc, transformed) {
	...
	return schema.options[key] === true || (typeof schema.options[key] === "function" && schema.options[key](doc, transformed));
}
...
function shouldHide({ options, schema, target, transformed }, doc, pathname) {
	...
	return testOptions(options, pathname) || testSchema(schemaType, hide, doc, transformed) || testSchema(schemaType, hideTarget, doc, transformed);
}
...
function shouldCopyVirtual({ options, schema, target }, key) {
	return schema.pathType(key) === "virtual" && [hide, `hide${target}`].indexOf(options.virtuals[key]) === -1;
}
...
function prepOptions(options, defaults) {
	...
	const ensure = ensureOption(options);
	options = {
