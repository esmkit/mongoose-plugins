...
mpathUtil.mongoSortToLodashSort = (mongoSortObj) => {
	...
	for (const key in mongoSortObj) {
		// eslint-disable-next-line no-prototype-builtins
		if (mongoSortObj.hasOwnProperty(key)) {
			const sortOrder = mongoSortObj[key] === -1 ? "desc" : "asc";
...
mpathUtil.listToTree = (list, sort) => {
	...
	let rootNodes = [];
	let index;
	const lodashSort = mpathUtil.mongoSortToLodashSort(sort);
...
export function mongooseMpath(schema, options) {
	...
	schema.pre("save", function preSave(next) {
		...
		const updateChildPaths = (pathToReplace, replacementPath) => {
			...
			const childStream = this.collection.find(childConditions).stream();

			const onStreamData = (childDoc, done) => {
				const newChildPath = replacementPath + childDoc.path.substr(pathToReplace.length);

				this.collection.updateMany({ _id: childDoc._id }, { $set: { path: newChildPath } }).then(() => done());
			};
		...
		if (this.parent) {
			this.collection
				...
				.then((parentDoc) => {
					const newPath = parentDoc.path + pathSeparator + this._id.toString();
					this.path = newPath;
	...
	schema.pre("remove", function preRemove(next) {
		...
		if ("DELETE" === onDelete) {
			...
			const childCursor = this.model(this.constructor.modelName).find(childConditions).cursor();
	...
	schema.statics.getChildrenTree = function getChildrenTree(args) {
		...
		const minLevel = args && args.minLevel ? args.minLevel : 1;
		const options = args && args.options ? args.options : {};
		const populateStr = args && args.populate ? args.populate : "";
